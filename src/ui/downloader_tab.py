...
# description: Sửa lỗi typo khiến nút bấm không được kích hoạt sau khi lọc.
...
class DownloaderTab(customtkinter.CTkFrame):
    ...
    def _create_treeview(self):
        ...
        self.tree.heading('title', text='Tiêu đề'); self.tree.heading('date', text='Ngày đăng'); self.tree.heading('url', text='URL')
        self.tree.column('title', width=400); self.tree.column('date', width=100, anchor='center'); self.tree.column('url', width=300)
    ...
    def process_scrape_queue(self):
        ...
        try:
            while not self.url_queue.empty():
                ...
                self.tree.insert('', 'end', values=("Đang chờ lấy chi tiết...", "", standard_url))
                self.update_status(f"Đã tìm thấy {len(self.scraped_links)} links...")
    ...
    def filter_and_fetch_worker(self, links_to_process, start_date, end_date):
        ...
        for i, link_info in enumerate(links_to_process):
            status_msg = f"Đang xử lý {i+1}/{total_links}... (Tìm thấy: {found_count})"
    ...
    def add_item_to_tree(self, item):
        # (Không đổi)
        self.tree.insert('', 'end', values=(item['title'], item['upload_date_str'], item['url']))

    def finalize_filtering(self, final_list):
        # (Logic sắp xếp và vẽ lại Treeview không đổi)
        self.video_details_list = sorted(final_list, key=lambda x: x.get('upload_date', '0'), reverse=True)
        self.tree.delete(*self.tree.get_children())
        for item in self.video_details_list: self.tree.insert('', 'end', values=(item['title'], item['upload_date_str'], item['url']))
        
        # --- hotfix - 2025-10-04 - Sửa lỗi typo 'final_li_list' thành 'final_list' ---
